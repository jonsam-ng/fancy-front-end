...
execHooksChain 执行链式的返回 Promise 的钩子函数，这里用的很巧妙，初始化为空的 Promise，在 Promise.then 中执行 hook。`Promise.resolve().then(() => {})` 返回一个 Promise，促使 reduce 继续执行。而且，这里 hook 的执行是非阻塞的，有此可见，这里的生命周期的钩子应该是顺序无关的。
...
以上是 Promise 类型定义，then 和 catch 返回的都是 Promise。

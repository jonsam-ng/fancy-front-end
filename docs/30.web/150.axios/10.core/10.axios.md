...
permalink: /axios/instance/index/
categories:
  - web
  - axios
  - core
...
事实上，在很多其他类型需要对每个行为进行横向扩展，如拦截、代理、任务链、过滤的情境中，都可以参考这种思想。同时这种链式处理思想在责任链设计模式、代理模式中都或多或少的有所体现，在 single-spa 等源码中也能看到类似的使用方法。
...
module.exports = function dispatchRequest(config) {
 ...
 // 合并公用的 headers 和 method 相关的 headers
 // Flatten headers
